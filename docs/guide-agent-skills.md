...
This **Salesforce Developer Experience (Salesforce DX)** repository does **not** vendor [forcedotcom/afv-library](https://github.com/forcedotcom/afv-library). Install the upstream bundle on your machine, then add this program’s small add-on skills pack.
...
- **Full Claude Code experience** (skills + hooks + **Language Server Protocol (LSP)** + agents):
  ...
  ```bash
  curl -sSL https://raw.githubusercontent.com/forcedotcom/afv-library/main/tools/install.sh | bash
  ```

- **Windows / continuous integration (CI) / no bash**:

  ```bash
  curl -sSL https://raw.githubusercontent.com/forcedotcom/afv-library/main/tools/install.py | python3
  ```
