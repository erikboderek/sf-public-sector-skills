...
Clerk Assist on-disk notes (`docs/status-agent-build.md` in the parent DX template) are optional for this installable repo—copy from the template or publish again if you want them on GitHub.
...
Repository root (as on [GitHub](https://github.com/erikboderek/sf-public-sector-skills/)):
...
`docs/` is populated when the parent template runs **`scripts/publish-skills-orphan.sh`**; it is **not** part of **`git subtree split --prefix=.cursor`**. You may trim files in this GitHub repo; the tree above reflects the **current** published layout.
