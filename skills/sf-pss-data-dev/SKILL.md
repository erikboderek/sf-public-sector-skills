...
| **Applications (draft / resume)** | `PreliminaryApplicationRef` | `PreliminaryApplicationRef` | Portal-side save-and-resume anchor for guest or authenticated intake; **`IsSubmitted`** + **`SubmissionDate`** flip on promotion to **`IndividualApplication`** or **`BusinessLicenseApplication`**. Do not hold authoritative program facts here beyond what the resume flow re-hydrates. |
